...
import { addOkr, updateOkr, getOkr } from "@Api/company_okr.js";
...
function OpForm({ record, open, closeModal, year }) {
  ...
  const handleOk = async () => {
    ...
    if (record?.id) {
      values.id = record.id;
      let { success, message: msg } = await updateOkr(values);
      if (success) {
        message.success(msg);
        closeModal(true);
      } else {
      ...
      let { success, message: msg } = await addOkr(values);
      if (success) {
        message.success(msg);
        closeModal(true);
      } else {
