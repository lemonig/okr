...
import {
  Layout, Button, Input, Select, Table, Spin
} from 'antd';
import './index.less'
import {
  PlusOutlined, EditOutlined, SnippetsOutlined, CloseOutlined,
  ExclamationCircleOutlined, DislikeOutlined, LikeOutlined, DislikeFilled, LikeFilled, UserOutlined
    ...
    <Layout className='matrix'>
      <Spin tip="Loading..." spinning={state.loading} >
        <Header className='bg-white' style={{ padding: '0' }}>
          <>
            <Select value={manchose} style={{ width: 120 }} onChange={handleChange} style={{ width: '320px' }}>
              {
                manlist.length && manlist.map(item => {

                  return <Option key={item.id} value={item.id}>{item.dept_1}-{item.dept_2}({item.nickname})</Option>
                })
              }
            </Select>
        ...
        <Content>
          <Table columns={columm} dataSource={tabledata} scroll={{ x: 1500, y: 620 }} bordered pagination={false} />
        </Content>
      </Spin>
    </Layout>
  )
