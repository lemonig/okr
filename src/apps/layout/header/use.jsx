...
import { Menu, Dropdown, Avatar, Modal, Form, Input, Button, Checkbox, message } from 'antd';
...
const User = () => {
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item key="3" icon={<EditOutlined />} onClick={handleModifyPwd}>修改密码</Menu.Item>
      <Menu.Divider />
    ...
    <div>
      <Dropdown overlay={menu} trigger={['click']}>
        <Avatar style={{ background: '#87d068' }}
          src={`http://grean-ops.oss-cn-hangzhou.aliyuncs.com/avatar/${JSON.parse(localStorage.getItem('user'))?.wxUserId}.png`}
          onClick={e => e.preventDefault()} />
        {/* <Avatar size={24} src="https://joeschmoe.io/api/v1/random" onClick={e => e.preventDefault()} /> */}
      </Dropdown>
      {/* 修改密码 */}
      <Modal title="修改密码" visible={pwdModalVisible} onOk={handlePwsOk} onCancel={handlePwsCancel} destroyOnClose>
        <Form
          name="psd"
          preserve={false}
          form={pwdForm}
